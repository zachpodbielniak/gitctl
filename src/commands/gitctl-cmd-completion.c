...
print_bash_completion(void)
{
    ...
    g_print(
        ...
        "    fi\n"
        "\n"
        "    local commands=\"pr issue repo release mirror api config completion\"\n"
        "    local global_flags=\"--help --version --license --dry-run --verbose --output --forge --remote --config\"\n"
        "\n"
        "    local verbs_pr=\"list get create edit close reopen merge checkout comment review browse\"\n"
        "    local verbs_issue=\"list get create edit close reopen comment browse\"\n"
        "    local verbs_repo=\"list get create delete fork clone browse\"\n"
        ...
        "    local flags_add=\"--url --direction --interval --sync-on-commit --token --username --create-repo --help\"\n"
        "\n"
        ...
        "        case \"${w}\" in\n"
        "            --output|--forge|--remote|--config|-o|-f|-r|-c)\n"
