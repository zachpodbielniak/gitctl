 ...
 * @argv: remaining argument vector after verb
 *
 * Handles "gitctl webhook delete <id>".  Requires --yes / -y
...
cmd_webhook_delete(
	...
	opt_context = g_option_context_new("<id> - delete a webhook");
	g_option_context_add_main_entries(opt_context, entries, NULL);
	...
	if (argc < 2)
	{
		...
		g_printerr("Usage: gitctl webhook delete [--yes] <id>\n");
		return 1;
	}
	...
	{
		g_printerr("warning: this will PERMANENTLY delete webhook '%s'\n",
		           webhook_id);
