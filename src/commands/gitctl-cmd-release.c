 ...
 * @argv: remaining argument vector after verb
 *
 * Handles "gitctl release delete <tag>".  Requires --yes / -y
...
cmd_release_delete(
	...
	opt_context = g_option_context_new("<tag> - delete a release");
	g_option_context_add_main_entries(opt_context, entries, NULL);
	...
	{
		g_printerr("error: release tag required\n");
		g_printerr("Usage: gitctl release delete [--yes] <tag>\n");
		return 1;
	}
	...
	{
		g_printerr("warning: this will PERMANENTLY delete release '%s'\n",
		           tag);
